...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const contactSelect = document.getElementById('contacto-por');
    const contactContainer = document.getElementById('otro-contacto-contenedor');
    const otroContactoInput = document.getElementById('otro-contacto');
    ...
    // Función para poblar las regiones y comunas
    ...
    regionSelect.addEventListener('change', (event) => {
        ...
        const regionSeleccionada = region_comuna.regiones.find(region => region.numero == event.target.value);
    ...
    // Lógica para mostrar/ocultar el input de "Otra" red social
    ...
    const validateName = (name) => {
        const lengthValid = name.trim().length >= 3 && name.trim().length <= 200;
        return lengthValid;
    };
    ...
    const validatePhoneNumber = (phoneNumber) => {
        // La validación solo se aplica si el campo no está vacío
        if (phoneNumber.trim() === "") return true;
        
        const lengthValid = phoneNumber.replace(/[\s\.]/g, '').length >= 8;
        const formatValid = /^(\+\d{1,3})?(\s)?(\d{1,3})?(\s)?(\d{8,11})$/.test(phoneNumber);
        return lengthValid && formatValid;
    };
    
    const validateOtherContact = (contact) => {
    ...
    const validateForm = () => {
        ...
        if (!validatePhoneNumber(celularInput.value)) {
            invalidInputs.push("Número de Celular");
        }
        if (contactSelect.value === 'otra' && !validateOtherContact(otroContactoInput.value)) {
            invalidInputs.push("ID de contacto (4-50 caracteres)");
        }
        ...
        if (!validateDeliveryDate(fechaEntregaInput.value)) {
            invalidInputs.push("Fecha de entrega (debe ser mayor o igual a la actual + 3 horas)");
        }
        if (!validatePhotos(document.querySelectorAll('input[type="file"]'))) {
        ...
        if (errores.length > 0) {
            ...
            if (confirmacion) {
                formulario.innerHTML = `
                    ...
                    <a href="index.html">
                        <button type="button">Volver a la portada del sistema</button>
                    </a>
                `;
            }
        }
